function optimizeImageUrl(url) {
    ...
    if (url.startsWith('http://') || url.startsWith('https://')) {
        ...
        return `https://res.cloudinary.com/dg3zclxrf/image/fetch/c_limit,w_1024,f_auto,q_auto/${encodeURI(url)}`;
    }
    
    const liveSiteUrl = 'https://mercandugunsalonu.com';
    const absoluteUrl = url.startsWith('/') ? `${liveSiteUrl}${url}` : `${liveSiteUrl}/${url}`;
    return `https://res.cloudinary.com/dg3zclxrf/image/fetch/c_limit,w_1024,f_auto,q_auto/${encodeURI(absoluteUrl)}`;
}
